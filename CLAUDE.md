...
| Manage knowledge base, ingest project results, query prior research | `/claudistotle:knowledge` |
...
- **`/feedback` routes by type**: Classifies advisor feedback into 方向性/文獻/論證/證據/寫作, then routes to the appropriate skill (/refine, /literature-review, /draft, /peer-review) with the feedback as additional context.
...
- `knowledge-base/` — Karpathy-style cross-project knowledge base. `raw/` stores project snapshots, `wiki/` contains LLM-maintained philosopher/concept/debate pages, `notes/` for informal research notes. Schema in `knowledge-base/CLAUDE.md`.
...
- `skills/knowledge/` — Knowledge base management: ingest project results into wiki, query existing knowledge. `scripts/` contains entity extraction tool.
- `skills/help/` — Interactive usage guide and tutorial. Teaches users how to use all Claudistotle skills effectively.
- `agents/` — Specialized subagent definitions invoked by the literature-review and knowledge skills. Includes `knowledge-ingest.md` for wiki enrichment.
