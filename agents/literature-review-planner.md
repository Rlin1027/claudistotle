...
Before planning domains and search strategies, identify the research methodology implied by the research question. Read `$CLAUDE_PLUGIN_ROOT/docs/references/philosophical-methods.md` if available, and adjust your planning accordingly:

- **Conceptual analysis** -> Prioritize PhilPapers, S2 with analytic philosophy journals; search for counterexamples and rival definitions
- **Hermeneutics / Phenomenology** -> Prioritize SEP/IEP for tradition context; include primary text scholarship; weight foundational thinkers heavily
- **Critical theory / Genealogy** -> Include interdisciplinary sources (sociology, political theory); search OpenAlex broadly
- **Comparative philosophy** -> Create parallel domains for each tradition; include cross-tradition scholarship as a separate domain
- **Empirical / Experimental philosophy** -> Include arXiv, cognitive science sources via S2 and OpenAlex

Also check for `reviews/[project-name]/research-proposal.md` — if it exists (generated by `/claudistotle:research-design`), read it to understand the research question, methodology, and expected contribution. Use this to inform domain decomposition.
