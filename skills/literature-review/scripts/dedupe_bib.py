...
def main():
    # Support both positional and --output flag styles:
    #   dedupe_bib.py output.bib input1.bib input2.bib ...
    #   dedupe_bib.py --output output.bib input1.bib input2.bib ...
    args = sys.argv[1:]
    ...
    while i < len(args):
        if args[i] in ("--output", "-o") and i + 1 < len(args):
            ...
            inputs_raw.append(args[i])
            i += 1

    # If --output was not used, first positional arg is output
    if output is None:
        if len(inputs_raw) < 2:
            print("Usage: dedupe_bib.py [--output] output.bib input1.bib [input2.bib ...]")
            sys.exit(1)
        output = Path(inputs_raw[0])
        inputs_raw = inputs_raw[1:]

    inputs = [Path(f) for f in inputs_raw]
